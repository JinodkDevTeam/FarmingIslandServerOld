...
use CLADevs\Minion\entities\types\FisherMinion;
...
class Main extends PluginBase{
	...
	public function onEnable(): void{
	    ...
	    Entity::registerEntity(FisherMinion::class, true);
	...
	public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool{
	    if($command->getName() === "minion"){
	        ...
	        if(!isset($args[1]) && (!$sender instanceof Player || !isset($args[0]))){
                $sender->sendMessage("Usage: /minion (miner|farmer|fisher) (player)");
                return false;
            }
    ...
    public static function asItem(string $type, Player $sender, int $level = 1, string $xyz = "n"): Item{
        ...
        if($type === FisherMinion::NAME){
            $minionType = "Fisher";
            $msgByType = "I will fishing if water around me";
        }
	    $lore = [];
	    foreach(
	        [
