...
class PureChat extends PluginBase
{
    ...
    */
    ...
    public function applyPCTags($string, Player $player, $message, $levelName)
    {
		...
		$string = str_replace("{prison_rank}", $this->getRankUpRank($player), $string);
    ...
    public function reloadNametag(Player $player)
    {
        $levelName = $this->getConfig()->get("enable-multiworld-chat") ? $player->getLevel()->getName() : null;

        $nameTag = $this->getNametag($player, $levelName);
